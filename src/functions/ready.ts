...
export default async function (client: CustomClient, ready: boolean): Promise<boolean> {
  ...
  setInterval(async () => {
    ...
    const parseBans: bans[] = await bans.findAll({ where: { reason: { [Op.like]: '%___irf' } } });
    for (const ban of parseBans) {
      // Extract data from ban
      let rblxUsername: string = ban.mod.discord;
      ...
      // Rewrite if FairPlay
      if (ban.mod.discord.includes('FairPlay')) rblxUsername = 'FairPlay_AntiCheat';
      // Fetch from Roblox
      moderator = await fetch('https://users.roblox.com/v1/usernames/users', {
        method: 'POST',
        body: JSON.stringify({ usernames: [rblxUsername] }),
