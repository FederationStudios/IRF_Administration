  ...
  run: async (client, interaction, fields) => {
    ...
    const embed = new Embed({
      ...
      fields: [
        ...
        {
          name: "RoWifi Link",
          value: rowifi.error ? `\`❌\` No, ${rowifi.error}` : `\`✅\` Yes, ${rowifi.roblox}`,
          inline: false
        },
        {
          name: "Offender",
    ...
    interaction.user.dmChannel.send({ content: `Here is a copy of a **MP report** you filed at <t:${Math.floor(Date.now()/1000.0)}:T>`, embeds: [embed] }); // DM copy
